...
class KlimPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('klim.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `klim`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM klim WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO klim (date, title, link, content)"
